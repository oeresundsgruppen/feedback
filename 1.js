...
Calendar._DN = new Array("Søndag", "Mandag", "Tirsdag", "Onsdag", "Torsdag", "Fredag", "Lørdag", "Søndag"); Calendar._SDN = new Array("S", "M", "T", "O", "T", "F", "L", "S"); Calendar._FD = 0; Calendar._MN = new Array("Januar", "Februar", "Marts", "April", "Maj", "Juni", "Juli", "August", "September", "Oktober", "November", "December"); Calendar._SMN = new Array("Jan", "Feb", "Mar", "Apr", "Maj", "Jun", "Jul", "Aug", "Sep", "Okt", "Nov", "Dec"); Calendar._TT = {}; Calendar._TT["INFO"] = "Om kalenderen"; Calendar._TT["ABOUT"] = "DHTML Date/Time Selector\n" + "(c) dynarch.com 2002-2005 / Author: Mihai Bazon\n" + "Se den nyeste version her:\n http://www.dynarch.com/projects/calendar/\n" + "Distribueret under GNU LGPL.\n Se flere oplysninger på http://gnu.org/licenses/lgpl.html." + "\n\n" + "Valg af dato:\n" + "- Brug knapperne \xab, \xbb til at vælge år\n" + "- Brug knapperne " + String.fromCharCode(0x2039) + ",  " + String.fromCharCode(0x203a) + " til at vælge måned\n" + "- Hold museknappen nede for hurtigere valg."; Calendar._TT["ABOUT_TIME"] = "\n\n" + "Valg af tid:\n" + "- Klik på en vilkårlig tidsdel for at øge tiden\n" + "- eller Shift-klik for at reducere tiden\n" + "- eller klik og træk for hurtigere valg."; Calendar._TT["PREV_YEAR"] = "Forri. år (hold nede for menu)"; Calendar._TT["PREV_MONTH"] = "Forri. måned (hold nede for menu)"; Calendar._TT["GO_TODAY"] = "Gå til i dag"; Calendar._TT["NEXT_MONTH"] = "Næste måned (hold nede for menu)"; Calendar._TT["NEXT_YEAR"] = "Næste år (hold nede for menu)"; Calendar._TT["SEL_DATE"] = "Vælg dato"; Calendar._TT["DRAG_TO_MOVE"] = "Træk for at flytte"; Calendar._TT["PART_TODAY"] = " (I dag)"; Calendar._TT["DAY_FIRST"] = "Vis %ss først"; Calendar._TT["WEEKEND"] = "0,6"; Calendar._TT["CLOSE"] = "Luk kalenderen"; Calendar._TT["TODAY"] = "I dag"; Calendar._TT["TIME_PART"] = "(Shift-)klik eller træk for at ændre værdien"; Calendar._TT["DEF_DATE_FORMAT"] = "%Y-%m-%d"; Calendar._TT["TT_DATE_FORMAT"] = "%b %e, %Y"; Calendar._TT["WK"] = "wk"; Calendar._TT["TIME"] = "Tid:";
...
var Prototype = { Browser: { IE: !!(window.attachEvent && navigator.userAgent.indexOf('Opera') === -1), Opera: navigator.userAgent.indexOf('Opera') > -1, WebKit: navigator.userAgent.indexOf('AppleWebKit/') > -1, Gecko: navigator.userAgent.indexOf('Gecko') > -1 && navigator.userAgent.indexOf('KHTML') === -1, MobileSafari: !!navigator.userAgent.match(/Apple.*Mobile.*Safari/) }, JSONFilter: /^\/\*-secure-([\s\S]*)\*\/\s*$/, emptyFunction: function () { } }; var Class = {
    create: function () {
...
if (!Array.prototype.each) Array.prototype.each = Array.prototype.iterate; Object.extend(Array.prototype, {
    clone: function () { return [].concat(this); }, indexOf: function (item, i) {
...
}); String.prototype.gsub.prepareReplacement = function (replacement) { if (Object.isFunction(replacement)) return replacement; var template = new Template(replacement); return function (match) { return template.evaluate(match) }; }; var Template = Class.create({
...
function $$() {
    var elements = new Array(); for (var i = 0; i < arguments.length; i++) {
        ...
        else if (arg.indexOf('.') > -1) { var foundElements = Selector.findElementsByClassName(arg); for (var j = 0; j < foundElements.length; j++) { elements.push(Element.extend(foundElements[j])); } }
        else { var foundElements = Selector.findElementsByTagName(arg); for (var j = 0; j < foundElements.length; j++) { elements.push(Element.extend(foundElements[j])); } }
    }
    return elements;
}
var Ajax = {
    _request: null, _requestObjects: [function () { return new XMLHttpRequest() }, function () { return new ActiveXObject('Msxml2.XMLHTTP') }, function () { return new ActiveXObject('Microsoft.XMLHTTP') }], initOptions: function (options) { this.options = { method: 'POST', asynchronous: true, contentType: 'application/x-www-form-urlencoded', encoding: 'UTF-8', parameters: '', evalJSON: true, evalJS: true }; for (option in options) { this.options[option] = options[option]; } }, getRequestObj: function () {
...
; (function () {
    var TestFieldLogic = Class.create({
        initialize: function () { }, initializeFocus: function () {
            var fields = $$(".field"); for (var i = 0; i < fields.length; i++) {
                if (fields[i].type == "radio" || fields[i].type == "checkbox") { fields[i].onclick = function () { fieldHighlight(this, 4); }; fields[i].onfocus = function () { fieldHighlight(this, 4); }; }
            ...
            Element.extend(fieldContainer); fieldContainer.addClassName("focused"); var focusedFields = $$(".focused"); for (var i = 0; i < focusedFields.length; i++) { if (focusedFields[i] != fieldContainer) { focusedFields[i].removeClassName("focused"); } }
        ...
        }, showRangeCounters: function () { var counters = $$("em.currently"); for (var i = 0; i < counters.length; i++) { counters[i].style.display = "inline"; } }, validateRange: function (ColumnName, RangeType) {
            var msg = $("rangeUsedMsg" + ColumnName); if (msg) {
                if (RangeType == "character") { var field = document.getElementById(ColumnName); msg.innerHTML = this.getCharacterMessage($(ColumnName)); }
                else if (RangeType == "word") { var field = document.getElementById(ColumnName); msg.innerHTML = this.getWordMessage(field); }
                else if (RangeType == "digit") { msg.innerHTML = this.getDigitMessage($(ColumnName)); }
            }
        }, getCharacterMessage: function (field) { return field.value.length; }, getWordMessage: function (field) {
            var val = field.value; val = val.replace(/\n/g, " "); var words = val.split(" "); var used = 0; for (var i = 0; i < words.length; i++) { if (words[i].replace(/\s+$/, "") != "") used++; }
            return used;
        }, getDigitMessage: function (field) { return field.value.length; }, ratingToggle: function (columnId) { if (document.observe) { this.setRatingNew(); } else { this.setRatingOld(); } }, setRatingNew: function () { var self = this; document.observe("click", function (e) { if (e.findElement(".rating")) { var el = e.findElement(".rating"), inputEl = el.getElementsByTagName("input")[0], currentRating = inputEl.getAttribute("value") || 0, newRating = e.target.getAttribute("value"), elems = el.getElementsByTagName("span"); self.toggleRatingElement(inputEl, newRating, currentRating, elems, "__MNG"); e.preventDefault(); } }); }, setRatingOld: function () { var self = this; $$(".rating").each(function (el) { var inputEl = el.getElementsByTagName("input")[0], elems = el.getElementsByTagName("span"); el.observe("click", function (event) { var $this; if (event.target) { $this = $(event.target); } else { $this = $(event.srcElement); }; var currentRating = inputEl.getAttribute("value"), newRating = $this.getAttribute("value"); self.toggleRatingElement(inputEl, newRating, currentRating, elems, "__PF"); return false; }); }); }, toggleRatingElement: function (inputEl, newRating, currentRating, elems, loc) {
            if (newRating == currentRating) { newRating = 0; }
            for (var i = 0, len = elems.length; i < len; i++) { if (elems[i].getAttribute("value") <= newRating) { elems[i].addClassName("clicked"); } else { elems[i].removeClassName("clicked"); } }
            if (newRating > 0) { inputEl.setAttribute("value", newRating); this.attachRatingRulesHandler(loc, inputEl); } else { inputEl.setAttribute("value", ""); this.attachRatingRulesHandler(loc, inputEl); }
        }, checkFileSize: function (evt) {
            var self = this; if (!window.File || !window.FileList) { return; }
            function onChangeHandler(el) {
                return function (evt) {
                    var $parentLi = $(el.parentNode.parentNode), file = evt.target.files[0]; $parentLi.removeClassName("error"); var errorMessages = $parentLi.getElementsByClassName("error"); if (errorMessages.length > 0) { errorMessages[0].parentNode.removeChild(errorMessages[0]); }
                    if (file && file !== undefined) { var size = file.size / 1000 / 1000; size = Math.round(size * 100) / 100; if (size > 10) { var errorMessage = "File must be below 10MB. That one was " + size + "MB!"; setLocalFieldError($parentLi, errorMessage); var clone = el.cloneNode(); $(clone).observe("change", onChangeHandler(clone)); el.parentNode.replaceChild(clone, el); if (clone.value) { clone.value = ''; } } }
                };
            }
            $$(".file").each(function (el) { el.observe("change", onChangeHandler(el)) });
        }, attachRatingRulesHandler: function (loc, el) { if (loc == "__PF") { __PF.handleInput(el); } else { __MNG.handleInput(el); }; }
    }); function setLocalFieldError($container, message) { var newError = document.createElement("p"); newError.setAttribute("class", "error"); newError.innerHTML = message; $container.addClassName("error"); $container.appendChild(newError); }
    window.TestFieldLogic = TestFieldLogic;
}());
var TestFormLogic = Class.create({
    offset: 0, startTime: 0, endTime: 0, loadTime: 0, initialize: function () { }, resizeCallFromParent: function (event) { if (event.data == 'resize') { this.initAutoResize(); } }, observeFormSubmit: function () {
        var activeForm = $$('form')[0]; var self = this; $(activeForm).observe('submit', function () { self.disableSubmitButton(); if (parent.postMessage) { parent.postMessage('formSubmitted', '*'); } }); if (typeof (Event) != 'undefined') { if (Event.observe) Event.observe(window, 'unload', function () { }); else Object.observeEvent(window, 'unload', function () { }); }
        ...
        var key = (typeof (__EMBEDKEY) != 'undefined') ? __EMBEDKEY : 'TestForm'; if (this.isEmbeddedForm() && key != 'false') {
        ...
        var childProxyFrameExist = false; try { var childProxyFrame = parent.frames["TestProxyFrame" + this.getFormHash()]; if (childProxyFrame.location.href.length > 0) { childProxyFrameExist = true; } }
        catch (e) { }
    ...
    }, getURLToParent: function () { var url = parent.frames['TestProxyFrame' + this.getFormHash()].location.href; url = url.substring(url.indexOf('#') + 1, url.length); return url; }, getFormHash: function () {
    ...
    }, addFragment: function (url, fragment, frameHeight) { url = this.removeFragment(url, fragment); url += '#' + fragment + '=' + frameHeight; return url; }, removeFragment: function (url, fragment) { var urlArray = url.split('#'); return urlArray[0]; }, saveHeightOnServer: function (name, value) { this.createTempCookie(name, value); document.body.appendChild(this.getScriptEl(name, value)); }, getScriptEl: function (name, value) { var rules = (this.hasRules()) ? 1 : 0; var script = document.createElement("script"); var src = document.location.protocol + "//Test.com/forms/height.js?action=set&embedKey="; src += name + "&height=" + value + "&rules=" + rules + "&protocol=" + document.location.protocol + "&timestamp=" + new Date().getTime().toString(); script.setAttribute("src", src); script.setAttribute("type", "text/javascript"); return script; }, hasRules: function () {
        var hasRules = false; if (typeof (__RULES) != 'undefined') { if (!this.isArray(__RULES)) { hasRules = true; } }
        return hasRules;
    }, isArray: function (object) { return object != null && typeof object == "object" && 'splice' in object && 'join' in object; }, createTempCookie: function (name, height) { var date = new Date(); date.setTime(date.getTime() + (60 * 1000)); var expires = "; expires=" + date.toGMTString(); var rules = (this.hasRules()) ? 1 : 0; var value = height + '|' + rules + '|' + document.location.protocol; document.cookie = name + "=" + value + expires + "; domain=.Test.com; path=/"; }, readTempCookie: function (name) {
        var nameEQ = name + "="; var ca = document.cookie.split(';'); for (var i = 0; i < ca.length; i++) { var c = ca[i]; while (c.charAt(0) == ' ') c = c.substring(1, c.length); if (c.indexOf(nameEQ) == 0) return c.substring(nameEQ.length, c.length); }
        return '';
    }
});
var TestConditions = Class.create({
    isEntryManager: false, isLiHidden: false, entry: '', initialize: function (isEntryManager, entry) { this.isEntryManager = isEntryManager; this.entry = entry; }, match: function (rule) {
        ...
        return ret;
    }, compare: function (condition, fieldTypes) { var fieldType = fieldTypes[condition.FieldName]; var fieldValue = this.cleanForComparison(this.getFieldValue(condition.FieldName, fieldType)); var conditionValue = this.cleanForComparison(condition.Value); var ret = (this.isLiHidden) ? false : this[condition.Filter.replace(/ /g, '')](conditionValue, fieldValue, fieldType); return ret; }, cleanForComparison: function (string) { string = string || ''; string = string.strip().stripTags().toLowerCase(); return string; }, getFieldValue: function (columnId, fieldType) {
        var field = this.getField(fieldType, columnId); var value = ''; this.isLiHidden = this.isFieldLIHidden(field); if (!this.isLiHidden) { value = this.getInputValue(this.verifyFieldType(fieldType, field), field, columnId); }
        return value;
...
var TestRuleLogic = Class.create({
    formId: '', valueHash: {}, hiddenHash: {}, isEntryManager: false, Rules: '', PublicForm: '', ConditionService: '', Entry: new Object(), RulesByConditionFieldName: new Object(), processAfterShowArray: new Object(), initialize: function (publicForm) { this.PublicForm = publicForm; this.initializeVariables(this.PublicForm.isEntryManager); this.attachFakeOnchangeToRadioButtons(); this.attachOnChangeToSelectFields(); }, initializeVariables: function (isEntryManager) {
        ...
        if (typeof (isEntryManager) != 'undefined') this.isEntryManager = isEntryManager; this.ConditionService = new TestConditions(this.isEntryManager, this.Entry);
    ...
    }, attachOnChangeToSelectFields: function () { if (this.attachOnChangeToSelect()) { var liInputs = (this.isEntryManager) ? $$('li select.field') : Selector.findElementsByTagAndClass(null, 'select', null); for (var i = 0; i < liInputs.length; i++) { liInputs[i].onchange = function () { handleInput(this); }; } } }, attachOnChangeToSelect: function () {
        if (navigator.userAgent.toLowerCase().indexOf('chrome') != -1) { return true; }
        else if (Prototype.Browser.MobileSafari) { return true; }
        else if (navigator.appVersion.indexOf("Win") != -1 && Prototype.Browser.WebKit) { return true; }
        else if (navigator.appVersion.indexOf("Mac") != -1 && Prototype.Browser.Opera) { return true; }
        else { return false; }
    }, organizeRulesByTargetField: function () { for (var fieldId in this.Rules) { var rules = this.Rules[fieldId]; for (var i = 0; i < rules.length; i++) { var rule = rules[i]; if (typeof (rule) != 'undefined') { if (this.formId == '') this.formId = rule.FormId; for (var j = 0; j < rule.Conditions.length; j++) { var c = rule.Conditions[j]; this.addToRulesByConditionFieldName(rule.Setting.FieldName, c.FieldName); } } } } }, addToRulesByConditionFieldName: function (key, value) {
...
var RunningTotal = Class.create({
    entry: '', decimals: 2, basePrice: 0, currency: '', totalText: '', basePriceText: '', publicForm: '', merchantFields: [], hiddenClassNames: ['rule_hide', 'hide'], tableTmpl: '<table id="run" border="0" cellspacing="0" cellpadding="0">' + '<tfoot>' + '#{rows}' + '</tfoot>' + '<tbody>' + '<tr><td colspan="2"><b>#{totalText}</b><span>#{total}</span></td></tr>' + '</tbody>' + '</table>', initialize: function (publicForm) { this.publicForm = publicForm; if (this.showRunningTotal()) { this.makeGlobalVariablesSafe(); this.entry = __ENTRY; this.currency = __PRICES.Currency; this.decimals = __PRICES.Decimals; this.totalText = __PRICES.TotalText; this.basePriceText = __PRICES.BasePriceText; this.basePrice = this.toNumber(__PRICES.BasePrice); this.organizeMerchantFields(__PRICES.MerchantFields); this.updateTotal(); this.calculateTop(); this.runLolaRun(); if (!document.getElementsByTagName('html')[0].hasClassName('embed')) { Object.observeEvent(window, 'scroll', this.runLolaRun); } } }, showRunningTotal: function () {
        ...
        this.merchantFields = new Array(); for (var i = 0; i < rawMerchantFields.length; i++) {
            ...
            else if (field.Typeof == 'radio' || field.Typeof == 'select') { var textToIndex = this.getTextToIndexs(field); for (var key in field.Choices) { if (typeof (field.Choices[key]) == 'object') { var index = textToIndex[field.Choices[key].Choice]; this.addToMerchantFields(field, field.Choices[key], index); } } }
            else if (field.Typeof == 'money') { this.addToMerchantFields(field, null, 0); }
        }
    }, getTextToIndexs: function (field) {
        var count = 0; var textToIndex = new Array(); for (var key in field.Choices) {
            var el = $('Field' + field.ColumnId + '_' + count); if (el) { textToIndex[el.value] = count; }
            count += 1;
        }
        return textToIndex;
        ...
        else if (field.Typeof == 'money') { $(field.ColumnId).value = $(field.ColumnId).value.replace(/[^0-9]/g, ''); $(field.ColumnId + '-1').value = $(field.ColumnId + '-1').value.replace(/[^0-9]/g, ''); var integer = $(field.ColumnId).value; var decimal = $(field.ColumnId + '-1').value; return new String(integer + '.' + decimal); }
...
var PublicForm = Class.create({
    formLogic: new TestFormLogic(), fieldLogic: new TestFieldLogic(), runningTotal: '', ruleLogic: '', formHeight: '', timerActive: false, genericInputs: {}, sortedTabindexes: [], isEntryManager: false, unableToChangeFile: 'We were unable to change your file.', initialize: function (runInit, isEntryManager) { this.isEntryManager = isEntryManager; if (runInit) this.runInit(); this.ruleLogic = new TestRuleLogic(this); this.runningTotal = new RunningTotal(this); }, runInit: function () { var redirectingToPaymentPage = this.continueToPaypal(); this.initCalendars(); this.formLogic.setLoadTime(); this.formLogic.observeFormSubmit(); this.fieldLogic.initializeFocus(); this.fieldLogic.showRangeCounters(); if (!redirectingToPaymentPage) this.formLogic.initAutoResize(0); this.setFormHeight(); this.handleTabs(); this.addOffsetForCaptcha(); this.formLogic.initAutoResize(this.formLogic.offset); this.fieldLogic.ratingToggle(); this.fieldLogic.checkFileSize(); }, addOffsetForCaptcha: function () { var IE6 = (navigator.userAgent.indexOf("MSIE 6") >= 0) ? true : false; if ($$('.captcha').length > 0 && IE6) { this.formLogic.offset = 30; } }, initCalendars: function () { var dateLIs = $$('.date', '.eurodate'); for (var i = 0; i < dateLIs.length; i++) { var startIndex = dateLIs[i].id.indexOf('li') + 2; var calendaFieldID = dateLIs[i].id.substring(startIndex); var onSelectFunction = (dateLIs[i].className.indexOf('eurodate') > -1) ? selectEuroDateOnForm : selectDateOnForm; Calendar.setup({ inputField: "Field" + calendaFieldID, displayArea: "cal" + calendaFieldID, button: "pick" + calendaFieldID, ifFormat: "%B %e, %Y", onSelect: onSelectFunction }); } }, handleTabs: function () {
        ...
        inputs = validInputs; var noTabIndexes = new Array(); var highestTabIndex = 1; for (var i = 0; i < inputs.length; i++) {
            var tabIndex = inputs[i].getAttribute('tabindex'); if (tabIndex > 0 && this.sortedTabindexes.indexOf(tabIndex) == -1) {
        ...
        var redirectingToPaymentPage = false; if ($('merchant')) {
            redirectingToPaymentPage = true; if ($('merchantButton')) { $('merchantMessage').innerHTML = $('merchantMessageText').innerHTML; $('merchantButton').style.display = 'none'; }
            $('merchant').submit();
        }
        return redirectingToPaymentPage;
    }, deleteFile: function (fieldId, file_name, container, removal, removeFile, formId) { Ajax.Request('/forms/File.Change.php', { parameters: "entryId=" + $('EntryId').value + "&fieldId=" + fieldId + "&fileName=" + file_name + "&formId=" + formId, onComplete: function (r) { var ret = r.responseText.evalJSON(); finishDeleteFile(ret, removeFile, removal, container); } }); }, failedDeleteFile: function (ret) { alert(this.unableToChangeFile); }, successfulDeleteFile: function (removeFile, removal, container) { $(removeFile).parentNode.removeChild($(removeFile)); $(removal).style.display = 'none'; $(container).style.display = 'block'; }, handleInput: function (el, count) {
...
var __PF; Object.observeEvent(window, 'load', init); Object.observeEvent(window, 'message', resizeCallFromParent); function init() { __PF = new PublicForm(true); }
...
function resizeCallFromParent(event) { if (__PF.formLogic) { __PF.formLogic.resizeCallFromParent(event); } }
function ratingToggle() { __PF.fieldLogic.ratingToggle(); }